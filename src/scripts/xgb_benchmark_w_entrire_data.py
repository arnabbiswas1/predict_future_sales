...
bst, best_iteration, valid_score, prediction = train.xgb_train_validate_on_holdout(
...
logger.info(f"Best iteration {best_iteration}, best validation score {valid_score}")
logger.info(f"Size of the prediction {len(prediction)}")
...
common.update_tracking(RUN_ID, "best_validation_score", valid_score)

submission = pd.DataFrame({'ID': test_df.ID, 'item_cnt_month': prediction})

logger.info("Saving submission file...")
common.save_file(logger, submission, constants.SUBMISSION_DIR, f"sub_{MODEL_NAME}_{RUN_ID}_xgb_baseline_full_training_data.csv")
