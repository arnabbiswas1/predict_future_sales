...
Final model is built using all data (including month 33) and final prediction is done on test data using that model.
"""
...
SEED = 42
EXP_DETAILS = "Benchamrk with CatBoost. item_cnt_month clipped between o to 20, validation on last month of training data (33). Final model is built using all data (including month 33) and final prediction is done on test data using that model."
IS_TEST = False
...
common.save_file(logger, submission, constants.SUBMISSION_DIR, f"sub_{MODEL_NAME}_{RUN_ID}_cat_baseline_full_training_data.csv")
